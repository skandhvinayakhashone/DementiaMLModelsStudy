...
stacked = data[['M/F','Hand']].stack()#get the stacked data of m/f and hand data
data[['M/F','Hand']] = pd.Series(stacked.factorize()[0], index=stacked.index).unstack()#factorize both the columns
...
k=0
maxi=0
#iterate through all neighbors in the range until maximum accuracy is obtained
classifier=KNeighborsClassifier(n_neighbors=19)#obtain the classifier funtion
